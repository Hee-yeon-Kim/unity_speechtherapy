...
public class MicrophoneListener : MonoBehaviour { 
    [SerializeField] private GameObject _imageSound; 
   // [SerializeField] private Text TextVol; 
    ...
    //option to toggle the microphone listenter on startup or not p
    ...
    //public to allow temporary listening over the speakers if you want of the mic output //but internally it toggles the output sound to the speakers of the audiosource depending //on if the microphone listener is on or off
...
void Update(){ //can use these variables that appear in the inspector, or can call the public functions directly from other scripts
 ...
 //아래는 커스텀 한 소스 
//소리가
    if (loudness < 1f) _imageSound.SetActive(true);
    else { _imageSound.SetActive(false); } 
